...
import br.com.taurustech.gestor.service.TipoPixService;
...
@RestController
@RequestMapping("/api/v1/tipo_pix")
@RequiredArgsConstructor
public class TipoPixController {
    private final TipoPixService service;
    ...
    public ResponseEntity<List<TipoPix>> listarTodos(){
        return ResponseEntity.ok(service.listarTodos());
    }

    @GetMapping("/{id}")
    @Secured({"ROLE_USER", "ROLE_ADMIN"})
    public ResponseEntity <TipoPix> getById(@PathVariable String id){
